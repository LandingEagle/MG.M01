...
Public Class mfcReadID3v2Tag
	...
	Public Shared Sub SearchForTracks(ByVal sStartFolder As String)
		Dim saFiles As String()
		Dim oTrack As mcTrack = Nothing

		saFiles = System.IO.Directory.GetFiles(sStartFolder, "*.mp3", IO.SearchOption.AllDirectories)

		If saFiles.GetUpperBound(0) > 0 Then

			For Each sFile As String In saFiles

				oTrack = ReadID3v2Tag(sFile)

				' Here is where you would want to search your database to 
				' match up the Artist Name with the Artists table.
				' If you find the artist in your database, update the oTrack 
				' object's .ArtistID with that PK. Do the same with the Album
				' and Track tables (Use the filename to distinguish new/existing tracks).
				' If you don't find a match, add it to the database.

				With frmMain.lvTracks.Items.Add(oTrack.Track)
					.SubItems.Add(oTrack.Album)
					.SubItems.Add(oTrack.Artist)
				End With
	...
	Public Shared Function ReadID3v2Tag(ByVal sFilename As String) As mcTrack
		Try
			...
			If (baHeader(0) = &H49) And (baHeader(1) = &H44) And (baHeader(2) = &H33) Then ' It's a proper MP3
				...
				' Start reading tags. I only use 4 now. There are more!
				oFrame = GetFrame(oFS)
...
#Region "Public Declarations"
	...
	Private pbBookmarked As Boolean = True
